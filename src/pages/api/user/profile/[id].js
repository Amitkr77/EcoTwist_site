...
export default async function handler(req, res) {
    ...
    try {
        if (req.method === "GET") {
            ...
            // Fetch user details
            const userToFetch = await User.findById(id).populate({
                path: 'wishlist',
                // populate: {
            ...
            if (decoded.id !== userToFetch._id.toString() && decoded.role !== "admin") {
                return res.status(403).json({ error: "Forbidden: You are not authorized to access this user profile" });
            }

            // Sanitize the user object by removing sensitive information
        ...
        if (req.method === "PUT") {
            ...
            if (decoded.id !== userToUpdate._id.toString() && decoded.role !== "admin") {
